...
from lib.pruning_mitigation import PruningMitigationTrojai
from lib.round21dataset import Round21Dataset
...
def demo_training_example_data(base_training_dataset_dirpath):
    ...
    for model_dir in os.listdir(training_models_dirpath):
        ...
        if os.path.exists(new_poisoned_data_example_dirpath):
            print(f"[DEMO] ======= Processing {new_poisoned_data_example_dirpath} =======")
            for example_file in os.listdir(new_poisoned_data_example_dirpath):
                if example_file.endswith('.png'):
                    print(f"[DEMO]  = = = File #{file_counter}: {example_file} = = = ")
                    ...
                    with open(example_groundtruth_filepath, 'r') as fp:
                        ...
                        print(f"[DEMO]    Image path: {example_image_filepath}")
                        print(f"[DEMO]    clean_label: {clean_label}")
                        print(f"[DEMO]    poisoned_label: {poisoned_label}")
        ...
        if os.path.exists(new_clean_data_example_dirpath):
            print(f"[DEMO] ======= Processing {new_clean_data_example_dirpath} =======")
...
def prepare_mitigation(args, config_json):
    ...
    mitigation = PruningMitigationTrojai(
        loss_cls=loss_class,
