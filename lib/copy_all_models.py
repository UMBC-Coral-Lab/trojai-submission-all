...
import torch
from lib.gcn_model import load_trafficnn_model
...
def copy_all_models(configure_models_dirpath, scratch_dirpath, logger):
    ...
    for folder in source_model_folders:
        ...
        if(poisoned):
            poisoned = "yes"
            new_model_file_name = f"round19_model{models_counter}_trojaned.pt"
        else:
            poisoned = "no"
            new_model_file_name = f"round19_model{models_counter}.pt"

        model_new = load_trafficnn_model(os.path.join(folder, "model.pt"))
