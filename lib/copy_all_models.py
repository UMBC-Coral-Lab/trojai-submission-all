...
def copy_all_models(configure_models_dirpath, scratch_dirpath, logger):
    ...
    for folder in source_model_folders:
        ...
        config_json_obj = config_json_obj["py/state"]

        model_arch = (((config_json_obj["model_architecture"].replace(" ", "_")).replace("-", "_")).replace("/", "_")).replace(":", "_")
        poisoned = config_json_obj["poisoned"]

        if(poisoned):
            poisoned = "yes"
            new_model_file_name = f"{model_arch}_model{models_counter}_trojaned.pt"
        else:
            poisoned = "no"
            new_model_file_name = f"{model_arch}_model{models_counter}.pt"
